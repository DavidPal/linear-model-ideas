...
\begin{abstract}
This document contains a few ideas about linear models.
\end{abstract}
...
Suppose that we would like to guarantee that $f:\R^d \to \R$ is a convex
function. Of course, a linear or an affine functions is convex, and it can be
found by doing least squares linear regression. A more general function is of
the form
$$
f(x) = a + b^T x + x^T C x
$$
...
can write it as
$$
\sum_{t=1}^T \left(y_t - a - b^T x_t - \ip{x_t x_t^T}{C}_F \right)^2
$$
...
$$
We would like the function $f(x,z)$ to be linear in parameter $x$. We don't
particularly care about how $f(x,z)$ dependens on $z$. A simple function that
satisfies this requirement has the form
$$
...
\subsection{Where is the model accurate?}

We say that the least squares model $\widehat{\theta}$ is \emph{accurate} on
an action $x \in \R^d$ if
$$
|(\widehat{\theta} - \theta)^T x| \le \alpha .
$$
In other words, the least squares model is accurate on $x$ if the prediction
$\widehat{\theta}^T x$ is close to true value $\theta^T x$.

We are interested in constructing a set of actions $x \in \R^d$ on which
$\widehat{\theta}$ is guaranteed to be accurate. For any $\alpha > 0$ andy
$\delta \in (0,1)$, we define the set $D_{\alpha,\delta}$ as
$$
D_{\alpha,\delta} = \{ x \in \R^d ~:~ 8 \sigma^2 x^T (X^TX)^{-1} x \ln(1/\delta) \le \alpha^2 \} \: .
$$
It can be proven that for any $x \in D_{\alpha,\delta}$, with probability at
least $1 - \delta$,
$$
|(\widehat{\theta} - \theta)^T x| \le \alpha \: .
$$
